...
```{r}
sqlStatement <- "SELECT pe.*, analytic.Admit_date
                from analytic, patient_encounter as pe 
                WHERE pe.patientID = analytic.patientid and 
                julianday(analytic.Admit_date) - julianday(pe.Actual_Date) < 180 AND
                pe.encounter_type = 48"
subQuery <- dbGetQuery(con, sqlStatement)
subQuery[1:30,]
```

Now that we only have ED visits that meet our criteria, we can just count the number of encounters for our `EScore`.  Additionally because `EScore` maxes out at 4, we need to use a `CASE` statement to make the score 4 if the number of visits is 4 or higher. 

```{r}
sqlStatement <- "SELECT patientID, 
                CASE WHEN COUNT(encounter_type) < 4 THEN COUNT(encounter_type) 
                ELSE 4 END as EScore
                FROM
                (SELECT pe.*, analytic.Admit_date
                from analytic, patient_encounter as pe 
                WHERE pe.patientID = analytic.patientid and 
                julianday(analytic.Admit_date) - julianday(pe.Actual_Date) < 180 AND
                pe.encounter_type = 48)
