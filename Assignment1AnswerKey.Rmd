...
sqlStatement <- "SELECT pe.*, 
                 ...
                 date(pe.Actual_Date) < date(analytic.Admit_date '- 6 month')"
...
```{r}
sqlStatement <- "SELECT a2.*, e.EScore FROM EDvisits as e LEFT JOIN analytic a2 ON a2.patientID = e.patientID"
