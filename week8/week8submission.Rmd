...
Your objective is to train two machine learning models - a logistic regression model and one other model of your choice. Previously, we only used a few select features. You may now use any or all features available to you; you do not need to try different combinations of variables (i.e. you just need to train one of each model).

Aside from the logistic regression model, you are free to use any of the models we've already covered OR a new model. First, we'll load the data and downsample it. Depending on which model you choose, you may need to normalize the feature space, too.
...
For the three following scenarios, would you prioritize sensitivity or specificity? An interpretable model or a "black box" model? Why?
