...
We'll be using the synthetic patient dataset, `cvd_patient`. Even though we performed a summary on this data last time, it's always good practice to reinspect in case anything has changed.
...
Notice above that we have an imbalanced dataset (i.e. there are far more people in our dataset *without* CVD than there are with CVD). For simpler machine learning models, this doesn't pose much of an issue. More advanced methods, however, will tend to overfit to the more-represented class. What does this mean in practice? It means our model would likely be very good at predicting when someone *doesn't* have CVD, but might suffer when they *do* have CVD.
...
Here, `cvd` is our outcome variable, and `age`, `gender` and `htn` are the predictors.
 
```{r fig.width=12}
## train the decision tree and plot its structure
decision_tree <- ctree(cvd ~ age + gender + htn, data = train_data, controls = ctree_control(minbucket = 15000, minsplit = 30000))
plot(decision_tree)
```

Here's the structure of our model. What we see is that out of the 7 variables in the training data, only `age` and `sbp` were most informative for predicting `cvd`. The probability bars at the terminal nodes tell us two things:
...
## for most machine learning tasks, you should normalize your feature vectors into the [-1, 1] or [0, 1] range
## first let's remember what our data looks like
head(train_data)

## now we normalize both the train and test datasets
...
## Training
Now we can begin building our model. Just so we can compare apples to apples, we'll only use the variables `age` and `sbp`, since they were identified as significant predictors in the decision tree model.

To begin, we tell `keras` that we're going to be building a sequential model (i.e. layer 1 feeds into layer 2, and layer 2 into layer 3, etc.). Then we use the pipe operator to add our layers. The first layer we'll add is a fully-connected dense layer consisting of 70 nodes. Each node will have the ReLU activation function applied to it. We then pass the information from that layer to the next layer, which contains 35 nodes that use the same activation function. That layer then passes to the final layer, which contains a single node with a sigmoid activation function, since we're performing binary classification.

You'll notice that in between each of the dense layers is a 50% dropout layer. What this does is, during training, randomly selects 50% of the weights and sets them to 0. This is a form of regularization that helps prevent the model from overfitting to the dataset.
...
## check the overall structure of the model
model %>% summary()
```
...
[talk about mapping the predictors - maybe use some dplr statements to make them more clear]

```{r, results='hide'}
## train the model
training <- model %>% fit(x = scaled_train[,c(2,6)], y = scaled_train[,1], validation_split = 0.20, batch_size = 5000, epochs = 25)
plot(training)
```
