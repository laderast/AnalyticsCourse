...
We'll be using the synthetic patient dataset, `cvd_patient`, which can be loaded from Dropbox. Even though we performed a summary on this data last time, it's always good practice to reinspect in case anything has changed.
...
Now we train our decision tree. To create a split (i.e. two diverging branches), we require there to be at least 30,000 samples in the node being split AND a terminal node can contain no less than 15,000 samples. We *could* allow smaller numbers here, but this could lead to overfitment of our model.
 
```{r fig.width=12}
## train the decision tree and plot its structure
decision_tree <- ctree(cvd ~ ., data = train_data, controls = ctree_control(minbucket = 15000, minsplit = 30000))
plot(decision_tree)
```
