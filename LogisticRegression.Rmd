...
$$
log(OddRatio(Readmit=TRUE)) = Constant + CoefA * LScore + CoefA * Ascore
$$

if we exponentiate both sides, remembering that `exp(A+B) = exp(A) * exp(B)`:

$$
`OddsRatio(Readmit=1) = exp(Constant + CoefL * LScore + CoefA * AScore)`
$$

Moving things around, we get:

$$
`OddsRatio(Readmit=1) = \frac{exp(prob(Readmit=1)}{1-prob(Readmit=1))} \\ = exp(Constant) * exp(CoefL* LScore) * exp(CoefA*AScore)`
$$

So we find that the `OddsRatio(Readmit = 1)` is calculated by multiplying `exp(Constant)`, `exp(CoefL * LScore)` and `exp(CoefA * AScore)`. This means that in order to interpret the coefficients in terms of odds, we need to exponentiate them. We can then interpret these transformed coefficients in terms of associated increase in the Odds Ratio. So let's first transform our coefficients by exponentiate them.
...
Now you have built a model. What next? If you want to see the values that the model predicts for the dependent variable, you can use `predict()`. This command will return two types of values, based on the arguments we pass it: 1) *predicted probabilities* and 2) the *Log of the Odds Ratio*
...
The area under the ROC curve (AUC) is one way we can summarize how well our model works. A model with perfect predictive ability has an AUC of 1. A random test (that is, a coin flip) has an AUC of 0.5. 
