...
laModel <- glm(Readmit30 ~ LScore + factor(AScore), family = "binomial", data=trainSet)
```
...
The interpretation of the `AScore` is different because we're treating it as categorical data (remember, if no admission through ER = 0, and admission through ER = 3). If there is an admission through emergency room, there is a `r expCoefs["factor(AScore)3"] * 100 - 100` percent increase in odds(Readmit score is 1).
