...
Do you want more info about how notebooks work? Here's a video: https://www.youtube.com/watch?v=TJmNvfhLCoI
...
When you start running code in a notebook, you'll see that you're generating output, such as graphs and tables. 

To share it, you'll have to *knit* it - which is a process of generating output using R and integrating them with the text. 

To download the file, click the checkbox for the file in the `Files` tab in the lower right, and click "More" in the menu bar underneath and select "Export". You'll get a file prompt and can then save your file to your computer.
...
When you open the notebook, the console window gets minimized as a bar below the script/notebook window. You can open it up by clicking the window icon in the lower left. Basically, all output from the console and the plot window (on the lower right) will get saved into your notebook.
...
con <- DBI::dbConnect(RSQLite::SQLite(), "data/patient1.sqlite")
```
...
The last thing we want to do is disconnect from our database. If we are reading from the database, this is not strictly necessary. When you are writing information into the database, disconnecting will save that information.
