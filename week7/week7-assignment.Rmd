...
I'm going to use a package called `skimr` to summarize the data:
...
The `glimpse` function is another useful tool to look at a `data.frame`:
...
We'll go this in detail with an example below.
...
The `geom_bar()` is the actual *geometric layout* that we are going to display the data on. 

## Getting more complicated: adding a `fill` aesthetic

We can get more complicated with mapping a variable to the fill column: 
...
```{r}
 bar_plot1 <-  ggplot(data=analytic_table, 
                      mapping=aes(x=factor(Readmit30), fill=Admit_source)) + 
...
```{r}
ggplot(data=analytic_table, mapping = aes(x=month(Discharge_date), fill=Admit_source)) + geom_bar()
```

## Calculating length of stay

Here I'm calculating the `length_of_stay` using the `mutate()` function, which lets me calculate a new variable based on the other variables. If we `glimpse()` our new table, we can see we have a new variabl called `length_of_stay`. What format is it in?

```{r}
analytic_table2 <- analytic_table %>% mutate(length_of_stay = Discharge_date - Admit_date)
glimpse(analytic_table2)
```

## Showing a histogram of length of stay

```{r}
hist_stay <- ggplot(data=analytic_table2, mapping = aes(x=length_of_stay, fill = outcome)) + geom_histogram(bins = 120) +
...
Now with `devtools` installed, we can install `patchwork`:
...
Now we can start grouping our plots together. Patchwork is good about scaling things to be 

```{r}
library(patchwork)

combined_graph <- hist_stay + bar_plot1 + plot_layout(ncol=1, heights = c(3,1))
