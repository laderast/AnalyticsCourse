...
analytic_table <- readr::read_csv(file = here::here("data/analytic.csv"), 
                                    col_types = cols(
...
analytic_table <- analytic_table %>% mutate(length_of_stay = Discharge_date - Admit_date) %>% filter(indexadmit ==1)
...
Show the `outcome` by month of `Admit_date` as a proportional plot for `analytic_table`. Save the result as `outcome_graph`. (Hint: think about what needs to be mapped to `x` and `fill`.) Make sure to add a title.

```{r}
library(lubridate)

outcome_graph <- analytic_table %>% ggplot(aes(x=month(Discharge_date), fill=outcome)) + geom_bar(position="fill", color="black") + ggtitle("Outcomes by Admit Month")
...
Hmm, based on the the graph, the majority of cases come from the emergency room. Even adjusting for that, it appears a large majority 

```{r}
analytic_table %>% ggplot(aes(x=Admit_source, fill=Readmit30)) + geom_bar(color="black")
...
heatmap_analytic <- ggplot(data=analytic_table_summarized, 
  ...
  scale_fill_gradient(low="white", high="lightblue", breaks=c( 10, 100, 1000, 5000)) +
...
Using a light color makes it a little easier to read. 
...
combined_graph <- outcome_graph + heatmap_analytic + 
